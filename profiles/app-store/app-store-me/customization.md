# Customization ladder — App Store (family `app-store-me`)
...
|---|---|---|
| **L0** Configure | yes | `spec.extraValues` → `apps/app-store-me/chart/values.yaml` |
| **L1** Drop-in | yes | `app-config` (`/etc/gentian/app-store-me/conf.d`, yaml) |
| **L2** Companion | always | the OpenAPI surface at `/api/v1` |
| **L3** Extension | **not yet** | adopt the template's entry-point loader (`gentian.app.app-store-me.plugins`) to unlock this — that change alone moves the app to grade A |
