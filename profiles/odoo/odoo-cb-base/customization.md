...
Applies to `odoo-cb-base` and every `odoo-cb-*` module profile, which inherit this declaration.
...
- **`module-profile`** — when the module is a catalogue *product*. Add a thin profile with
  `deployment-role: module` and `requires-profile: odoo-cb-base` (the `odoo-cb-crm` pattern).
